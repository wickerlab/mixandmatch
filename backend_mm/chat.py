...
def insert_message(sender_id, receiver_id, message, status, cnx):
    with cnx.cursor() as cursor:
        query = (
            "INSERT INTO message (sender_id, receiver_id, message, update_time, status)"
            " VALUES (%s, %s, %s, NOW(), %s)"
        )
...
async def chat_server(websocket, path):
    ...
    try:
        ...
        async for message in websocket:
            ...
            print(
                f"Received message from {sender_id} to {receiver_id}: {chat_message}, {read_status}"
            )
            ...
            with cnx.cursor() as cursor:
                ...
                cursor.execute(
                    query, (sender_id, receiver_id, chat_message, read_status)
                )
                cnx.commit()
                print("message saved")
...
start_server = websockets.serve(chat_server, "localhost", 8765, ping_interval=None)
